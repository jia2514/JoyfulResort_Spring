...
public class RoomScheduleService {
	...
	public String getByPeopleAmount(Map<String, String[]> map) {
		Map<String, String> query = new HashMap<>();
		Set<Map.Entry<String, String[]>> entry = map.entrySet();
		int peopleAmount = 0;
		for (Map.Entry<String, String[]> row : entry) {
...
//		System.out.println("type3Empty+"+type3Empty);
		List<Map<Integer, Integer>> listRoomSchedule = new ArrayList<>();
...
//		System.out.println("twoPeopleRoom+"+twoPeopleRoom);
		...
		if(fourPeopleRoom>0 && twoPeopleRoom>0) {
			if(type2Empty>=fourPeopleRoom && type1Empty>=twoPeopleRoom) {
				...
				element.put(1, twoPeopleRoom);
				element.put(2, fourPeopleRoom);			
				listRoomSchedule.add(element);
			}
			if(type3Empty>=fourPeopleRoom && type1Empty>=twoPeopleRoom) {
				Map<Integer, Integer> element = new HashMap<>();
				element.put(1, twoPeopleRoom);
				element.put(3, fourPeopleRoom);			
				listRoomSchedule.add(element);
			}
			if(type2Empty<fourPeopleRoom && type3Empty<fourPeopleRoom && type2Empty+type3Empty>=fourPeopleRoom && type1Empty>=twoPeopleRoom){
				Map<Integer, Integer> element1 = new HashMap<>();
				element1.put(1, twoPeopleRoom);
				element1.put(2, type2Empty);
				element1.put(3, (fourPeopleRoom-type2Empty));			
				listRoomSchedule.add(element1);
				
				Map<Integer, Integer> element2 = new HashMap<>();
				element2.put(1, twoPeopleRoom);
				element2.put(2, (fourPeopleRoom-type3Empty));
				element2.put(3, type3Empty);			
				listRoomSchedule.add(element2);
				
			}else if(type2Empty+type3Empty == (fourPeopleRoom-1) && type1Empty-twoPeopleRoom==2) {
				Map<Integer, Integer> element1 = new HashMap<>();
				element1.put(1, twoPeopleRoom+2);
				element1.put(2, type2Empty);
				element1.put(3, (fourPeopleRoom-type2Empty-1));			
				listRoomSchedule.add(element1);
				
				Map<Integer, Integer> element2 = new HashMap<>();
				element2.put(1, twoPeopleRoom+2);
				element2.put(2, (fourPeopleRoom-type3Empty-1));
				element2.put(3, type3Empty);			
				listRoomSchedule.add(element2);
			}
			...
			if(type2Empty<fourPeopleRoom && type3Empty<fourPeopleRoom && type2Empty+type3Empty>=fourPeopleRoom){
				...
				if(type2Empty==0) {
					Map<Integer, Integer> element = new HashMap<>();
					element.put(1, 2);
					element.put(3, (fourPeopleRoom-1));			
					listRoomSchedule.add(element);
				}else if(type3Empty==0) {
					Map<Integer, Integer> element = new HashMap<>();
					element.put(1, 2);
					element.put(2, (fourPeopleRoom-1));			
					listRoomSchedule.add(element);
				}else {
					Map<Integer, Integer> element1 = new HashMap<>();
					element1.put(1, 2);
		...
		String jsonStr = new JSONArray(listRoomSchedule).toString();
		System.out.println("117 jsonStr+"+jsonStr);
		return jsonStr;
	}
